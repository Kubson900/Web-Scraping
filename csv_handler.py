...
class CsvHandler:
    ...
    def create_csv_distinct_records(self):
        with open(f'{self.product.name}/{self.product.name}_distinct_records.csv', 'w', encoding="utf-8", newline='') as file:
    ...
    def create_csv_basic_price_info_by_city(self):
        with open(f'{self.product.name}/{self.product.name}_basic_price_info_by_city.csv', 'w', encoding="utf-8", newline='') as file:
            writer = csv.writer(file, delimiter=';')
